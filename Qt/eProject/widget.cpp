    ...
    , ui(new Ui::Widget)
{
    ...
    restart_timer.setInterval(2000);   // 2 sec
...
void Widget::on_dishRgBt_released()
{
    ...
    // get order list
    int dishNum = 5;  // will from dish recognition
...
void Widget::on_faceRgBt_clicked()
{
    ...
    int UsrIdx = 1;  // will from dish recognition: faceRecognize()
...
{
    ui->faceLdImg->setVisible(false);
    ui->statusHd->setText(" Account Found!");
...
void Widget::make_payment(int UsrIdx)
{
...
int Widget::faceRecognize()
{
    ...
    while(UsrIdx == -1)
    {
        ...
        //Mat* face_arr = new Mat[faces.size()];    //定以数组
        string str;
        for (int i = 0; i < faces.size(); i++)
        {
            Mat face = windowFrame(faces[i]);
            //face_arr[i] = face; //将所有的脸部保存起来
            //if (face_arr[i].empty())
              //  continue;
