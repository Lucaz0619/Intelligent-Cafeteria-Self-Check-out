...
void Widget::onRecognitionDone()
{
    int UsrIdx = faceRecog->UserIdx;
    if (UsrIdx == -1)
    {
        qDebug()<<UsrIdx;
...
void Widget::on_dishRgBt_released()
{
    ...
    faceRecog = new FaceRecognition(cap);
    //connect FaceRecognition class signal to Widget class slot
    connect(faceRecog, SIGNAL(recognitionDone()), this, SLOT(onRecognitionDone()));

//    faceRecog = new FaceRecognition(cap);
//    int UsrIdx = faceRecog->recognize();
//    //int UsrIdx = faceRecognize();  // will from dish recognition: faceRecognize()
//    if (!faceRecog->recognitionFinished())
//    {
//        qDebug()<<UsrIdx;
//        qDebug()<<"recognize fail!";
//    }
...
void Widget::readDishFrame()
{
    ...
    //ui->label_camera->setPixmap(QPixmap::fromImage(imag.mirrored(true,false)));   // mirror the image
}
